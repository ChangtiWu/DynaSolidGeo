function visual(mode, azimuth, elevation, point_A, point_B, point_C, point_A1, point_B1, point_C1, point_E, point_F)
    close all;
    ...
    all_points = {A, B, C, A1, B1, C1, E, F};
    labels = {point_A, point_B, point_C, point_A1, point_B1, point_C1, point_E, point_F};
    for i = 1:length(all_points)
        pt = all_points{i};
        scatter3(pt(1), pt(2), pt(3), 40, 'ko', 'filled');
        if strcmp(labels{i}, point_B)
             text(pt(1)-0.2, pt(2)-0.2, pt(3), labels{i}, 'FontSize', 14, 'FontWeight', 'bold');
        else
             text(pt(1)+0.1, pt(2)+0.1, pt(3), labels{i}, 'FontSize', 14, 'FontWeight', 'bold');
        end
    end
