function visual(mode, azimuth, elevation, point_P, point_A, point_B, point_C, point_D, point_E, point_F)
    ...
    plot3([P(1), E(1)], [P(2), E(2)], [P(3), E(3)], 'r--', 'LineWidth', 1.5);
    plot3([C(1), F(1)], [C(2), F(2)], [C(3), F(3)], 'b-', 'LineWidth', 1.5);
    
    patch([P(1), A(1), B(1)], [P(2), A(2), B(2)], [P(3), A(3), B(3)], 'g', 'FaceAlpha', 0.2);
    patch([P(1), C(1), D(1)], [P(2), C(2), D(2)], [P(3), C(3), D(3)], 'm', 'FaceAlpha', 0.2);
    
    all_points = {A, B, C, D, P, E, F};
    labels = {point_A, point_B, point_C, point_D, point_P, point_E, point_F};
    ...
    text(B(1)-0.2, B(2)+0.1, B(3), point_B, 'FontSize', 12, 'FontWeight', 'bold');
    text(C(1)+0.1, C(2)+0.1, C(3), point_C, 'FontSize', 12, 'FontWeight', 'bold');
    text(D(1)+0.1, D(2)-0.2, D(3), point_D, 'FontSize', 12, 'FontWeight', 'bold');
    text(P(1), P(2)-0.2, P(3)+0.1, point_P, 'FontSize', 12, 'FontWeight', 'bold');
    text(E(1)+0.1, E(2)-0.2, E(3), point_E, 'FontSize', 12, 'FontWeight', 'bold');
    text(F(1)+0.1, F(2)-0.2, F(3), point_F, 'FontSize', 12, 'FontWeight', 'bold');
