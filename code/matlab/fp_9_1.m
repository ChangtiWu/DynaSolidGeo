...
function visual(mode, azimuth, elevation, point_A, point_B, point_C, point_D, point_A1, point_B1, point_C1, point_D1, point_P, point_M, point_N)
    ...
    A1 = [0, 0, side_length];
    B1 = [side_length, 0, side_length];
    C1 = [side_length, side_length, side_length];
    D1 = [0, side_length, side_length];
    ...
    plot3([P(1), N(1)], [P(2), N(2)], [P(3), N(3)], 'k--', 'LineWidth', 1.5);
    
    
    plot3([M(1), N(1)], [M(2), N(2)], [M(3), N(3)], 'k--', 'LineWidth', 1.5);
    
    
    scatter3(A(1), A(2), A(3), 80, 'ko', 'filled');
    scatter3(B(1), B(2), B(3), 80, 'ko', 'filled');
    scatter3(C(1), C(2), C(3), 80, 'ko', 'filled');
    scatter3(D(1), D(2), D(3), 80, 'ko', 'filled');
    scatter3(A1(1), A1(2), A1(3), 80, 'ko', 'filled');
    scatter3(B1(1), B1(2), B1(3), 80, 'ko', 'filled');
    scatter3(C1(1), C1(2), C1(3), 80, 'ko', 'filled');
    scatter3(D1(1), D1(2), D1(3), 80, 'ko', 'filled');
    ...
    text(M(1)+0.2, M(2)+0.2, M(3)+0.1, point_M, 'FontSize', 14, 'FontWeight', 'bold');
    text(N(1)-0.2, N(2)+0.2, N(3)+0.1, point_N, 'FontSize', 14, 'FontWeight', 'bold');
