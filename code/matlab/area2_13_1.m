function visual(mode, azimuth, elevation)
    ...
    surf(X_cone, Y_cone, Z, 'FaceColor', [0.7, 0.8, 0.9], 'FaceAlpha', 0.4, 'EdgeColor', 'none');
    
    
    
    x_cyl_bottom_fill = [0; r * cos(theta')];
    y_cyl_bottom_fill = [0; r * sin(theta')];
    z_cyl_bottom_fill = zeros(size(x_cyl_bottom_fill));
    fill3(x_cyl_bottom_fill, y_cyl_bottom_fill, z_cyl_bottom_fill, [0.9, 0.7, 0.8], 'FaceAlpha', 0.4, 'EdgeColor', 'none');
    
    
    x_cyl_top_fill = [0; r * cos(theta')];
    y_cyl_top_fill = [0; r * sin(theta')];
    z_cyl_top_fill = (H/2) * ones(size(x_cyl_top_fill));
    fill3(x_cyl_top_fill, y_cyl_top_fill, z_cyl_top_fill, [0.9, 0.7, 0.8], 'FaceAlpha', 0.4, 'EdgeColor', 'none');
    
    
    [THETA_cyl, Z_cyl] = meshgrid(theta, [0, H/2]);
    X_cyl = r * cos(THETA_cyl);
    Y_cyl = r * sin(THETA_cyl);
    surf(X_cyl, Y_cyl, Z_cyl, 'FaceColor', [0.9, 0.7, 0.8], 'FaceAlpha', 0.4, 'EdgeColor', 'none');
