...
function visual(mode, azimuth, elevation, point_A, point_C, point_B, point_D, point_P)
    ...
    angle_ABD = 30 * pi/180;  
    BD_length = 2/sqrt(3);
    D_original = [-BD_length * sin(angle_ABD), BD_length * cos(angle_ABD), 0];
    D_original = [-BD_length * 0.5, BD_length * sqrt(3)/2, 0];  
    ...
    P = [-0.3, BD_length * sqrt(3)/2, BD_length * 0.5];  
    ...
    plot3([A(1), D_original(1)], [A(2), D_original(2)], [A(3), D_original(3)], 'k--', 'LineWidth', 2);  
    plot3([B(1), D_original(1)], [B(2), D_original(2)], [B(3), D_original(3)], 'k--', 'LineWidth', 2);  
    ...
    scatter3(D_original(1), D_original(2), D_original(3), 100, 'ko', 'filled');
    ...
    text(D_original(1)+0.1, D_original(2), D_original(3), point_D, 'FontSize', 14, 'FontWeight', 'bold');
