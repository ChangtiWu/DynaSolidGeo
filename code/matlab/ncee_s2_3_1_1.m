function visual(mode, azimuth, elevation, point_A, point_B, point_C, point_D, point_F, point_E)
    ...
    A = [-ab_len/2, bc_len/2, 0];
    B = [ab_len/2, bc_len/2, 0];
    C = [ab_len/2, -bc_len/2, 0];
    D = [-ab_len/2, -bc_len/2, 0];
    E = [x0, 0, h];
    F = [-x0, 0, h];
    ...
    for i = 1:length(all_points)
        ...
        text(pt(1) * 1.05, pt(2) * 1.05, pt(3) + 0.3, labels{i}, 'FontSize', 12, 'FontWeight', 'bold');
    end
