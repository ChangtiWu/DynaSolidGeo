...
json_data = {
    ...
    "type": 7,
    "level": 2,
    "cn_problem": f"设正方体 {point_A}{point_B}{point_C}{point_D}-{point_A1}{point_B1}{point_C1}{point_D1} 的棱长为 {len_a}，{point_P} 是对角面 {point_B}{point_D}{point_D1}{point_B1}（包含边界）内一点，满足 {point_P}{point_A} ⊥ {point_P}{point_C}。过 {point_P} 作平面 α 与直线 {point_P}{point_C} 垂直，求：1. 平面 α 与平面 {point_A}{point_B}{point_C}{point_D} 所成锐二面角的最小值；",
...
with open(matlab_cmd_jsonl_path, "a", encoding="utf-8") as f:
    f.write(json.dumps({json_data["id"]: f"zht_14_3_1({mode}, {azimuth}, {elevation}, '{point_A}', '{point_B}', '{point_C}', '{point_D}', '{point_A1}', '{point_B1}', '{point_C1}', '{point_D1}', '{point_P}')"}, ensure_ascii=False) + "\n")
