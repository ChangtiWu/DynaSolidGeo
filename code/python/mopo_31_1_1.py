...
def calculate(len_a):
    ...
    len_d_min = (3 * math.sqrt(5) * len_a) / 5
...
json_data = {
    ...
    "type": 7,
    "level": 2,
    "cn_problem": f"在棱长为 ${len_a}$ 的正方体 {point_A}{point_B}{point_C}{point_D}-{point_A1}{point_B1}{point_C1}{point_D1} 中，{point_E} 为棱 ${point_B}{point_C}$ 的中点，{point_P} 是底面 {point_A}{point_B}{point_C}{point_D} 内的一点（包含边界），且 ${point_B1}{point_P} \\\\perp {point_D1}{point_E}$，求线段 ${point_B1}{point_P}$ 长度的最小值。",
...
with open(matlab_cmd_jsonl_path, "a", encoding="utf-8") as f:
    f.write(json.dumps({json_data["id"]: f"mopo_31_1_1({mode}, {azimuth}, {elevation}, '{point_A}', '{point_B}', '{point_C}', '{point_D}', '{point_A1}', '{point_B1}', '{point_C1}', '{point_D1}', '{point_E}', '{point_P}')"}, ensure_ascii=False) + "\n")
