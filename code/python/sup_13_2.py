...
def calculate(len_a, len_h, len_m, len_n):
    ...
    float: 计算结果（二面角的正弦值）
    """
    # 根据题干解法：sinθ = sqrt(((len_h - len_n)^2 + len_a^2) / ((len_m - len_n)^2 + (len_h - len_n)^2 + len_a^2))
    numerator = (len_h - len_n) ** 2 + (len_a ** 2)
    denominator = (len_m - len_n) ** 2 + (len_h - len_n) ** 2 + (len_a ** 2)
...
json_data = {
    ...
    "type": 2,
    "level": 2,
    "cn_problem": f"在三棱柱{point_A}{point_B}{point_C}-{point_A1}{point_B1}{point_C1}中，{point_C}{point_C1}⊥平面{point_A}{point_B}{point_C}，{point_A}{point_C}⊥{point_B}{point_C}，设{point_A}{point_C}={point_B}{point_C}={len_a}，{point_C}{point_C1}={len_h}，点{point_D}在棱{point_A}{point_A1}上且{point_A}{point_D}={len_m}，点{point_E}在棱{point_C}{point_C1}上且{point_C}{point_E}={len_n}。求二面角{point_B}-{point_B1}{point_E}-{point_D}的正弦值。",
...
with open(matlab_cmd_jsonl_path, "a", encoding="utf-8") as f:
    f.write(json.dumps({json_data["id"]: f"sup_13_2({mode}, {azimuth}, {elevation}, '{point_A}', '{point_B}', '{point_C}', '{point_A1}', '{point_B1}', '{point_C1}', '{point_D}', '{point_E}')"}, ensure_ascii=False) + "\n")
