...
def calculate(len_a, len_b, len_h):
    """计算长度len_d的值（表达式为分子2len_b²len_h除以分母平方根）"""
    numerator = 2 * (len_b ** 2) * len_h
    denominator_sq = 4 * (len_b ** 4) + (len_h ** 2) * ((len_a ** 2) + 2 * len_a * len_b + 2 * (len_b ** 2))
...
with open(matlab_cmd_jsonl_path, "a", encoding="utf-8") as f:
    f.write(json.dumps({json_data["id"]: f"ncee_b_6_1_3({mode}, {azimuth}, {elevation}, '{point_A}', '{point_B}', '{point_C}', '{point_D}', '{point_A1}', '{point_B1}', '{point_C1}', '{point_D1}', '{point_N}', '{point_M}')"}, ensure_ascii=False) + "\n")
