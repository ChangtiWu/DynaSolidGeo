...
json_data = {
    ...
    "type": 7,
    "level": 2,
    "cn_problem": f"设正方体 {point_A}{point_B}{point_C}{point_D}-{point_A1}{point_B1}{point_C1}{point_D1} 的棱长为 ${len_a}$，{point_E}、{point_F} 分别是棱 ${point_B}{point_C}$、${point_C}{point_C1}$ 的中点。动点 {point_P} 在正方形 ${point_B}{point_C}{point_C1}{point_B1}$（包括边界）内运动，若 ${point_P}{point_A1} \\\\parallel$ 平面 ${point_A}{point_E}{point_F}$，求线段 ${point_P}{point_A1}$ 的最大长度。",
    "en_problem": f"Let cube {point_A}{point_B}{point_C}{point_D}-{point_A1}{point_B1}{point_C1}{point_D1} have edge length ${len_a}$, and {point_E}, {point_F} are midpoints of edges ${point_B}{point_C}$ and ${point_C}{point_C1}$ respectively. If moving point {point_P} lies within square ${point_B}{point_C}{point_C1}{point_B1}$ (including boundary) and ${point_P}{point_A1} \\\\parallel$ plane ${point_A}{point_E}{point_F}$, find the maximum length of segment ${point_P}{point_A1}$.",
    "solution": f"{result}",
...
with open(matlab_cmd_jsonl_path, "a", encoding="utf-8") as f:
    f.write(json.dumps({json_data["id"]: f"mopo_9_1_2({mode}, {azimuth}, {elevation}, '{point_A}', '{point_B}', '{point_C}', '{point_D}', '{point_A1}', '{point_B1}', '{point_C1}', '{point_D1}', '{point_E}', '{point_F}', '{point_P}')"}, ensure_ascii=False) + "\n")
