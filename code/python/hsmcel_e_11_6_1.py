...
json_data = {
    ...
    "type": 8,
    "level": 3,
    "cn_problem": f"设正六边形 ${point_A}{point_B}{point_C}{point_D}{point_E}{point_F}$ 的边长为 ${len_a} > 0$。沿对角线 ${point_A}{point_D}$ 折叠，使平面在 ${point_A}{point_D}$ 处折成二面角 ${point_M}\\text{{-}}{point_A}{point_D}\\text{{-}}{point_N}$。当折后使直线 ${point_C}{point_F}$ 与 ${point_A}{point_D}$ 的夹角为 $ arg_alpha  = 45°$ 时，求该二面角的大小 $ arg_phi $。",
...
with open(matlab_cmd_jsonl_path, "a", encoding="utf-8") as f:
    f.write(json.dumps({json_data["id"]: f"hsmcel_e_11_6_1({mode}, {azimuth}, {elevation}, '{point_A}', '{point_B}', '{point_C}', '{point_D}', '{point_E}', '{point_F}', '{point_M}', '{point_N}')"}, ensure_ascii=False) + "\n")
