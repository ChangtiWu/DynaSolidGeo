...
def calculate(len_a, arg_alpha, arg_theta_0):
    """
    计算翻折后点 O 到直线 AC 距离的最大值

    参数:
    len_a (float): 菱形边长
    arg_alpha (float): ∠BAD 的角度（弧度）
    arg_theta_0 (float): θ 的下界（弧度）

    返回:
    float: 距离最大值
    """
    # 根据题干给出的解答公式: len_a * cos(arg_alpha/2) * cos(arg_theta_0/2)
    return len_a * math.cos(arg_alpha / 2) * math.cos(arg_theta_0 / 2)


# 定义题干中的参数变量
len_a = 8.0                     # 菱形边长
arg_alpha = math.radians(60)    # ∠BAD = 60°
arg_theta_0 = math.radians(90)  # θ 的下界 90°
arg_theta_1 = math.radians(120) # θ 的上界 120°

# 验证计算结果（确认与参考答案一致后注释掉）
# result = calculate(len_a, arg_alpha, arg_theta_0)
...
with open(matlab_cmd_jsonl_path, "a", encoding="utf-8") as f:
    f.write(json.dumps({json_data["id"]: f"fz2_7_2({mode}, {azimuth}, {elevation}, '{point_A}', '{point_B}', '{point_C}', '{point_D}', '{point_O}')"}, ensure_ascii=False) + "\n")
